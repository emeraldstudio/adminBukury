...
export class ConductorComponent implements OnInit {
	...
	constructor(private ws: WebserviceService,private storage: AngularFireStorage,private modalService: NgbModal,private activatedRoute: ActivatedRoute, private db: AngularFireDatabase) {
		...
		model.item.subscribe((res) => {
			...
			if (res.ubicacion) {
			    ...
			    const posMar = {lat: model.latitude, lng: model.longitude};
				const marker = new google.maps.Marker({position: posMar, map: model.map});
			}

		});


		const itemsRef = db.list('Conversaciones/' + model.key);
    ...
    documentoSubido(event) {
	    ...
	    const filePath = 'documentos/' + model.key + '/' + model.tipoDoc + '.png';
